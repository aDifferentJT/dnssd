...
namespace pml::dnssd
{
    ...
    bool ServiceBrowser::StartBrowser()
    {
        ...
        pml::log::log(pml::log::Level::kError, "BonjourBrowser") << "Failed to start";
        return false;
    }
    ...
    void ServiceBrowser::RunSelect()
    {
        ...
        while(m_bRun)
        {
            ...
            if ( result > 0 )
            {
                pml::log::log(pml::log::Level::kDebug, "pml::dnssd::BonjourBrowser") << "BonjourBrowser: Select done" ;
                //
            ...
            {
                pml::log::log(pml::log::Level::kDebug, "pml::dnssd::BonjourBrowser") << "Result = " << result;
                break;
            }
            if ( count > 10 )
            {
        ...
        if ( flags & kDNSServiceFlagsAdd && !errorCode )
        {
            ...
            if(itServicePost != m_mServiceBrowse.end())
            {
                ...
                if ( itService == m_setServiceTypes.end() )
                {
                    ...
                    {
                        pml::log::log(pml::log::Level::kError, "pml::dnssd::BonjourBrowser") << "Error trying to browse service type: " << service_type.c_str();
                    }
                }
            }
        }
    ...
    void DNSSD_API ServiceBrowser::IterateInstances( DNSServiceRef sdRef, DNSServiceFlags flags, uint32_t interfaceIndex, DNSServiceErrorType errorCode, const char *serviceName, const char *regtype, const char *replyDomain, void *context )
    {
        if ( (flags & kDNSServiceFlagsAdd) && !errorCode )
        {
            ...
            if ( itServToString != m_mServRefToString.end() )
            {
                ...
                {
                    pml::log::log(pml::log::Level::kDebug, "pml::dnssd::BonjourBrowser") << "BonjourBrowser: Error trying to browse service instance: " << serviceName;
                }
            }
            else
            {
            ...
            if ( itInstance != m_mInstances.end() )
            {
                ...
                }
                {
                    ...
                    if ( err == 0 )
                    {
                        ...
                        pml::log::log(pml::log::Level::kDebug, "pml::dnssd::BonjourBrowser")  << "BonjourBrowser: Looking up " << itInstance->second->sHostName << " on " << sAdapter;
                    }
                    else
                    {
